...
const sendMail = async ({ name, email, message, subject }: TypeData) => {
  ...
  //   to: "samuraior.dev@gmail.com", //Change to email address that you want to receive messages on
  //   subject: subject,
  //   html: HTML_TEMPLATE(message, email, name),
  // };
  const options = {
    from: "Web personal samuraior.dev <samurai.dev.ordiales@gmail.com>",
    to: "samuraior.dev@gmail.com",
    subject: subject,
    html: HTML_TEMPLATE(message, email, name),
    envelope: {
      from: "samuraior.dev@gmail.com",
      to: "amurai.dev.ordiales@gmail.com",
    },
  };
  try {
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  if (req.method === "POST") {
    ...
    if (result.success) {
      return res
        .status(200)
        .json({ message: "Correo electrónico enviado", status: true });
    } else {
    ...
    res
      .status(405)
      .json({ message: "Solo se permiten solicitudes POST", status: false });
  }
};
